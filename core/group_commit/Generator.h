...
public:
  ...
  void prepare_transactions_to_run(WorkloadType& workload, StorageType& storage){
    ...
    for (auto i = 0u; i < query_num; i++) {
      ...
      transactions_queue.push_back(txn);
  ...
  void start() override {
    ...
    for (;;) {
      ...
      size_t batch_size = transactions_queue.size();
      for(size_t i = 0; i < batch_size; i ++ ){
        simpleTransaction txn = transactions_queue.front();
...
protected:
  ...
  std::deque<simpleTransaction> transactions_queue;
