...
//    DCHECK((int)partition_owner_cluster_worker(partition_id) == this_cluster_worker_id);
  ...
  bool commit_transaction(TransactionType &txn,
    ...
    }
    {
      ...
      if (this->context.hstore_active_active && is_replica_worker == false) {
        ...
        release_locks_async(txn, messages, false);
      } else {
        txn.message_flusher();
      }
    }
  ...
  void record_commit_transactions(TransactionType &txn){
    ...
    this->txn_statics.add(txn.b);

    this->total_latency.add(std::chrono::duration_cast<std::chrono::microseconds>(
  ...
  void process_mp_transactions(std::vector<TransactionType*> & mp_txns) {
    {
      for (size_t i = 0; i < mp_txns.size(); ++i) {
        ...
        execute_transaction(mp_txns[i]);
          ...
          int time_local_locks = std::chrono::duration_cast<std::chrono::microseconds>(
                                                                    txnStartTime - mp_txns[i]->b.execStartTime)
                  .count();
        mp_txns[i]->b.time_local_locks = time_local_locks;
        //handle_requests(false);
      }
    }
    ...
    if (sp_txns.empty() == false) {
      ...
      if (sp_parallel_exec_commit) {
        ...
        for (size_t i = 0; i < sp_txns.size(); ++i) {
          ...
          // {
            // ScopedTimer z0([&, this](uint64_t us) {
            //   txn->b.time_local_locks += us;
            // });
            
            txn->reset();
            txn->b.execStartTime = std::chrono::steady_clock::now();
            if (!process_single_transaction(txn)) {
              txn->abort_lock = true;
              this->n_abort_lock.fetch_add(1);
            }

            txn->b.time_local_locks = std::chrono::duration_cast<std::chrono::microseconds>(
                                                                    std::chrono::steady_clock::now() - txn->b.execStartTime)
                  .count();
          // }
        }
