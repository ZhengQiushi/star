...
template <class Workload, class Protocol> class SiloGenerator : public Worker {
public:
  ...
  SiloGenerator(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
    ...
    while (!in_queue.empty()) {
      ...
      for (auto it = message->begin(); it != message->end(); it++) {
        ...
        if(type < controlMessageHandlers.size()){
          // transaction router from SiloGenerator
