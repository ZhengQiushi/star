...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=24 --partitioner=Lion --threads=4 --batch_size=10000 --batch_flush=500   --neworder_dist=100 --lion_with_metis_init=1 --time_to_run=200   --sample_time_interval=3 --migration_only=0 --n_nop=0 --v=5 --data_src_path_dir="/home/star/data/tpcc/s80_d100_30/"  --read_on_replica=true  --random_router=0 --workload_time=100 --lion_self_remaster=0 --repartition_strategy=clay --skew_factor=80 --rn_nop=40000' % (id, cmd, protocol))
