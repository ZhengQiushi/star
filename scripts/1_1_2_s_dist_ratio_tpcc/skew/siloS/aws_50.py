...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/bench_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=24 --threads=4 --partitioner=hash2 --batch_size=10000 --batch_flush=500 --lion_with_metis_init=0 --time_to_run=60 --workload_time=60 --sample_time_interval=3 --migration_only=1 --neworder_dist=50 --skew_factor=80' % (id, cmd, protocol))
