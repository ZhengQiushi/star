...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=24 --threads=4 --partitioner=hpb --granule_count=1000 --hstore_command_logging=true --wal_group_commit_time=1000 --wal_group_commit_size=0 --batch_size=10000 --batch_flush=500 --lion_with_metis_init=0 --time_to_run=240 --workload_time=60 --sample_time_interval=3 --replica_group=2 --lock_manager=1  --neworder_dist=100 --lotus_async_repl=true --log_path=/tmp --skew_factor=0' % (id, cmd, protocol))
