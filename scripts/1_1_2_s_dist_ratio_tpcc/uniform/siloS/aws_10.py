...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partitioner=Lion --partition_num=24 --threads=4 --neworder_dist=0 --batch_size=10000 --batch_flush=500 --lion_with_trace_log=false --data_src_path_dir="/home/star/data/tpcc/skew_0_30/" --v=5 --time_to_run=60 --workload_time=60 --sample_time_interval=3 --n_nop=20000 --lion_with_metis_init=0 --migration_only=0 --read_on_replica=false --random_router=0 --skew_factor=0' % (id, cmd, protocol))
