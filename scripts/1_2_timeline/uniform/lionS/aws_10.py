...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=12 --partitioner=Lion --threads=4 --batch_size=10000 --batch_flush=500   --cross_ratio=0 --lion_with_metis_init=0 --time_to_run=120   --sample_time_interval=3 --migration_only=0 --n_nop=800000 --v=5 --data_src_path_dir="/home/star/data/skew_50_50/"  --read_on_replica=true  --random_router=0 --workload_time=60 --lion_self_remaster=0  --skew_factor=0' % (id, cmd, protocol))
