...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=12 --partitioner=Lion --threads=4 --cross_ratio=100 --batch_size=10000 --batch_flush=500 --lion_with_trace_log=false --data_src_path_dir="/home/star/data/tpcc/skew_0_30/" --v=7 --time_to_run=240 --workload_time=60 --sample_time_interval=3 --n_nop=200000 --lion_with_metis_init=0 --migration_only=1 --read_on_replica=true --random_router=0 --skew_factor=80 --rn_nop=40000' % (id, cmd, protocol))
