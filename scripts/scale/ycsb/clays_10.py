...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/bench_ycsb --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=%d --partitioner=Lion --threads=2 --batch_size=10000 --batch_flush=500   --cross_ratio=100 --lion_with_metis_init=1 --time_to_run=160   --sample_time_interval=3 --migration_only=1 --n_nop=0 --v=5 --data_src_path_dir="/home/star/data/ycsb/ips%d/"  --read_on_replica=true  --random_router=0 --workload_time=80 --lion_self_remaster=0 --repartition_strategy=clay' % (id, cmd, protocol, partition_num, n - 1))
