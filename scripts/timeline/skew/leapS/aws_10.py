...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/bench_ycsb --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=12 --threads=4 --partitioner=Lion --batch_size=10000 --batch_flush=500 --lion_with_metis_init=0 --time_to_run=60 --workload_time=60 --sample_time_interval=3 --migration_only=1 --cross_ratio=0 --skew_factor=80  --read_on_replica=false  --random_router=0' % (id, cmd, protocol))
