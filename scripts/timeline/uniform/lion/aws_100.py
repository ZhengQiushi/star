...
for protocol in protocols: 
  for i in range(len(ratios)):
    ratio = ratios[i]
    cmd = get_cmd(n, i)
    print('/home/star/zqs_laji --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=12 --threads=4 --partitioner=Lion --batch_size=10000 --batch_flush=500 --lion_with_metis_init=1 --time_to_run=240 --workload_time=60 --sample_time_interval=3 --migration_only=0 --cross_ratio=100 --skew_factor=0  --read_on_replica=true  --random_router=0' % (id, cmd, protocol))
